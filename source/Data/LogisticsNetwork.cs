...
public static class LogisticsNetwork
{
    ...
    public static LogisticsProvider AddProvider(ObjectInfo oi, ResourceDefinition rd, double minimumKeep, string networkId = "", bool toOrbit = false)
    {
        var data = GetOrCreate(oi);
    ...
    public static Dictionary<string, int> GetAllLVTypeCountsOnObject(ObjectInfo oi, Company player)
    {
        ...
        foreach (var lv in Object.FindObjectsOfType<LaunchVehicle>())
        {
            ...
            if (!IsLVAvailableForLogistics(lv, player)) continue;
    ...
    public static bool IsLVAvailableForLogistics(LaunchVehicle lv, Company player)
    {
        if (lv == null) return false;
        var oid = lv.objectInfo?.GetObjectInfoData(player);
        if (oid == null) return true;
        var facility = oid.GetFakeLVFromFacilityReverse(lv);
        if (facility == null) return true;
        return oid.IsFacilityEnabled(lv);
    }
    ...
    public static ObjectInfo GetLowOrbitOf(ObjectInfo body)
    {
        return body?.LowOrbitCustom?.GetObjectInfo();
    }
